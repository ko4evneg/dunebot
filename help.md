...
[Рейтинг](#rating)\
<b>[Как использовать бота](#examples)</b>
...
выслыается
отчет с победителями и результатами всех участвовавших в матче игроков. Данный бот существует для того, чтобы этот
рейтинг
работал в автоматическом режиме.
...
`/help` Помощь\
_<pre>В реализации: Выдавать все доступные команды в виде кнопок по единой команде /commands </pre>_
\
[В начало...](#home)
...
ботом: `/register Имя (ник_steam) Фамилия`. При регистрации бот сохранит для рейтинга ваши данные и чат, в который будет
в
дальнейшем отправлять вам уведомления. К сожалению, всем текущим игрокам придется зарегистрироваться еще раз, так как в
текущих
...
Изменить данные игрока возможно командой `/refresh_profile Имя (ник_steam) Фамилия` (ваши имя и фамилия для рейтинга).
Даже если вы хотите поменять только
одно имя, вписывать необходимо все три имени (те, которые не меняете, в этом случае совпадут с указанными при
регистрации).
Также эту команду необходимо выполнить для обновления данных у бота, если вы меняете данные профиля в Telegram,
например,
ник или имя (обратите внимание, это **не то же самое**, что имя для участия в рейтинге, а именно в телеграме, в самой
команде нужно вводить **имя и фамилию для рейтинга**, данные из телеграма бот вытащит самостоятельно) - **иначе у вас не
будут работать уведомления бота типа @telegramName**.
_<pre>В реализации: обновления профиля телеграм командой '/refresh_profile' без указания имен.</pre>_

[В начало...](#home)

## <a id="create-poll"/> Создание опроса

Для создания опроса выполните команду `/new dune` (классика) или `/new up4` (апрайзинг на 4-х). Когда в опросе будет 4
игрока, начнется минутный отсчет, по истечении которого, если из опроса никто
