...
[Исправление ошибочных результатов](#result-resubmit)\
...
Команду может выполнить любой игрок, участвующий в матче (сабмитер). При выполнении команды, этому игроку придет два
запроса:
в первом будут кнопки с именами всех игроков, участвовавших в опросе матча, во втором - кнопки всех лидеров. В обоих
запросах,
сабмитер должен нажать четыре кнопки с именами игроков и лидеров в порядке занятых ими мест, начиная от 1 места и
заканчивая
4 местом.\

**Пример:** игроки с именами А, Б, В, Г заняли такие места: А-3,Б-2,В-4,Г-1. Тогда кнопки нажать нужно будет в порядке
Г-Б-А-В.

Если игрок не закончит процедуру сабмита, матч завершится без результата и в чат будет отправлено соответствующее
уведомление.
До этого будет выслано предупредительное уведомление с тегом сабмитера.
По истечение таймаута, отведенного на исправление ошибочных результатов, матч будет завершен с уведомлением в общий чат.

[В начало...](#home)

## <a id="result-resubmit"/> Исправление ошибочных результатов

После регистрации результатов сабмитером, все игроки получат уведомление с информацией об их местах и лидерах. Если
игрок
видит ошибку, он может запустить повторную регистрацию результатов командой `/resubmit ID_игры`. После выполнения
команды,
игроку придет запрос с выбором:

- если игрок помнит результаты всех участников матча и их лидеров, он может выполнить повторную регистрацию
  самостоятельно
- в иных случаях, он может попросить выполнить регистрацию предыдущего сабмитера

По итогам этого выбора, нужному игроку придет запрос на регистрацию результатов.\
В случае 3 неудачных попыток регистрации, матч будет завершен без сохранения результата.
...
Выбор мест игроков, нужно выбрать всех участников, начиная с 1-го и заканчивая 4-м местом
![alt text](help/submit_reply.jpg)

Сабмитер получит предварительный результат и возможность выбрать лидеров, в таком же порядке: от 1 до 4 места:\
![alt text](help/submit_reply2.jpg)

После публикации лидеров, сабмит завершается и сабмитер получает такое сообщение:\
![alt text](help/submit_reply3.jpg)

А все отслаьные игроки получают такое сообщение и могут выполнить команду /resubmit, чтобы оспорить назначенное место:\
![alt text](help/submit_reply4.jpg)
